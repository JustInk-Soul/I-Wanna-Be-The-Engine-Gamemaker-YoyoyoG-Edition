...
if (!settingKey) { // Check if we're currently setting a key bind
    ...
    if (keyboard_check_pressed(vk_anykey)) { // Check if a key is being pressed and if it is, store that key
        ...
        settingKey = false;
    } else if (Input_AnyControllerButtonPressed() != -1) { // If a controller button was pressed, exit prompt
        settingKey = false;
    }
}

// Update current control strings
strKey[0] = Input_GetString(global.leftButton[0]);
strKey[1] = Input_GetString(global.rightButton[0]);
strKey[2] = Input_GetString(global.upButton[0]);
strKey[3] = Input_GetString(global.downButton[0]);
strKey[4] = Input_GetString(global.jumpButton[0]);
strKey[5] = Input_GetString(global.shootButton[0]);
strKey[6] = Input_GetString(global.restartButton[0]);
strKey[7] = Input_GetString(global.skipButton[0]);
strKey[8] = Input_GetString(global.suicideButton[0]);
strKey[9] = Input_GetString(global.pauseButton[0]);
strKey[10] = "";